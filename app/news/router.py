...
@news.get('/my')
async def get_my_news(db: AsyncSession = Depends(get_session),
                      author: User = Depends(get_current_user)) -> schemas.NewsList:
    return schemas.NewsList(
        news=await service.get_users_news(db, author)
    )
